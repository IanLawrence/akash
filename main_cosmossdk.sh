...
if [[ -n $SSH_PASS ]] # If the SSH_PASS variable is set, enable the SSH service
then 
apt install -y ssh
...
then
	...
	git clone $GITHUB_REPOSITORY && cd $GIT_FOLDER && git checkout $BINARY_VERSION 
	make build
	make install
